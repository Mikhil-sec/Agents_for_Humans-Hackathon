...
## 12 September 2026 — the demo's centrepiece card, and the script beats that could not be filmed

**Done**

- **The live decision card is now FitLife, not PhotoCloud.** Yorvan's 11 Sept script audit
  assumed the centrepiece rendered as a gym card with wrong narration. It did not render
  as a gym card at all — the shipped pending card was *PhotoCloud, £4.99*, with a
  one-line evidence panel, while the video opens on "£456 for a gym I stopped going to".
  The two scenarios are swapped in `replay.py`: PhotoCloud is now the week 1 decision and
  FitLife is week 4, so the card a judge opens the demo on is the one the pitch is about.
- **The swap is count-preserving and the curve is untouched.** Week 1's `tag_merchant`
  silent action moved to PhotoCloud with it, so the measured result is identical:
  **4/2/1/1 decisions and 3/5/5/6 autonomous**, 85.71% autonomy, £164.20 saved. The
  policy engine still measures the curve; nothing here authors it.
- **The card now carries the whole argument in its own evidence.** The week 4 signal
  reads *"Your annual membership renews in 6 days at GBP 456.00 for the year. No visit
  recorded since 12 March; GBP 38.00 has been taken every month since."* The policy
  preview reads *"Always cancel subscription for FitLife up to GBP 38.00"* — the monthly
  charge, deliberately, because `hooks.py` reads `monthly_amount_minor` for both the card
  amount and the rule ceiling, and a rule written against £456 would be a rule about the
  wrong number.
- **`docs/DEMO_SCRIPT.md` 1:25–2:40 rewritten.** Three beats in "the most important 75
  seconds" described things the product does not do, and only one of them was in Yorvan's
  list:
  - *"The Today screen. **Empty.**"* — it ships reading **"One thing needs you."** with
    the card visible. The empty state now lands at 2:25 as the payoff for answering,
    which is a better beat than opening on it unexplained.
  - *"Trigger a run. Cards appear."* — **no card can ever appear.** A fixtures-mode run
    completes immediately with all-zero stats, and the stream filters pending decisions
    to the new `run_id`, which matches nothing. Beat removed; the script now says not to
    press "Check now" on camera.
  - *"It found three things"* — one, as Yorvan said.
  - *"already drafted the cancellation"* — nothing is drafted before approval; a CONFIRM
    tool interrupts before it executes. Reworded.
- **The two false claims are fixed.** "Deployed on AgentCore Runtime" is replaced with
  "built for", plus the zero-credential line, and carries a `**Do not say**` note with
  the support-case number so a future reader does not restore it. "Sitting durably in S3"
  → "parked durably", since mock mode uses the local store.

**Verified how**

- Ran the product, not just the tests. Every claim above was checked on screen:
  the card, its expanded evidence panel, the policy preview, the Rules page (7 rules,
  PhotoCloud now the week 1 one), the Insights chart (still 4→1, 3→6, axis 1/8/15/22 Aug),
  and the empty state after answering.
- Confirmed "Check now" is inert two ways: `POST /api/runs` returns `decisions_raised: 0`
  with no pending card bound to the new run, and clicking it on the Today page leaves the
  DOM unchanged. Both backends do this (`fixtures.py`, `staticBackend.ts`); there is no
  third backend that runs the agent.
- Confirmed the card resets to pending on a demo restart, so a second take is possible.
- **The hosted demo is live** — `https://mikhil-sec.github.io/Agents_for_Humans-Hackathon/`
  returns 200 and renders fully for a logged-out visitor. The checklist item can be ticked
  and the video's "live demo linked below" line is safe to say.
- 307 tests pass (214 agent, 50 integrations, 43 api). `ruff check` and `ruff format
  --check` clean on `replay.py`.

**Notes for other lanes**

- **`/fixtures` has moved** — `decisions.json`, `activity.json`, `policies.json`,
  `runs.json` and `daily_brief.json` were regenerated by the exporter. Most of the diff is
  the usual uuid churn from non-deterministic ids; the semantic change is only the
  PhotoCloud/FitLife swap. No Lane C input files were touched.
- **The Activity page stamps every entry with the export time**, so all 26 sit under one
  day heading rather than spreading over four weeks. Pre-existing, cosmetic, and the same
  family as the run-timestamp bug already fixed in `export_fixtures.py`. The Insights
  chart carries the four-week axis correctly, so the script makes the time argument there
  instead. Not fixed — flagging rather than changing the exporter on freeze eve.

**Blocked / needs a human**

1. **The Devpost deadline is still wrong in the checklist**, in two places —
   `SUBMISSION_CHECKLIST.md` line 3 and the assignment table both say "15 September, in
   the morning". Yorvan says Devpost closes 14 Sept 17:00 PDT (01:00 BST on the 15th).
   Nothing in this repo records the authoritative time, so I have not edited it — someone
   needs to open the Devpost rules page and confirm. If he is right, a Monday-morning
   submission misses it by about eight hours.
2. The video, the blog post and the submission text description remain Mikhil's, and
   none are started. The script can now be read as written.
